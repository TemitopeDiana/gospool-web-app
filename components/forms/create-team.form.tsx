...
const defaultValues: CreateTeamFormInput = {
  name: '',
  churchLogo: null,
  adminFirstName: '',
  adminLastName: '',
...
const CreateTeamForm = ({ close }: { close: () => void }) => {
  const [isPending, setIsPending] = useState(false);
  const form = useForm<CreateTeamFormInput>({ defaultValues });
  const { handleSubmit } = form;

  const onSubmit: SubmitHandler<CreateTeamFormInput> = async (data) => {
    setIsPending(true);
